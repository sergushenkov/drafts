Первая версия двухсвязного списка написана в рамках изучения структур данных. Писал по TDD, но в момент написания теста уже было понимание, какие будут операции внутри того или иного метода, и структура теста фактически следовала за структурой кода. Т.е. в каждом тесте проверялось что код меняет именно те данные, которые должен и не затрагивает те, что не должны измениться. В итоге получился довольно простой код и довольно громоздкий тест.

Вторая версия кода была написана сначала в виде спецификаций, потом под эти спецификации (логический уровень программы) были написаны тесты и под них же (спецификации) был написан код. Спецификации, разбиение методов на сетеры и гетеры привело к усложнению структуры двухсвязного списка, появлению дополнительных методов. Первая версия тестов была создана под Unittest, вторая - под Pytest, но не это причина того что тесты имеют разный формат.  Заметное упрощение тестов и большая их наглядность вызвана именно .тем, что тесты следовали за спецификациями. В проверках использовались только публичные методы класса, без прямых обращений к скрытым от пользователя атрибутам. Плюс то, что структура данных, следуя за спецификациями, получила дополнительные гетеры, возвращающие аварийные флаги - также сильно упростило тестирование.

В целом, написание кода через спецификации, сильно проще, чем без них. С одной стороны кажется что это дополнительная затрата времени, с другой стороны в итоге времени уходит меньше. Проще ещё без кода продумать структуры, методы, их ограничения - и потом легко и быстро реализовать их, чем исправлять код на ходу, когда понимаешь, что одно из условий в первоначальном плане противоречит другому.
